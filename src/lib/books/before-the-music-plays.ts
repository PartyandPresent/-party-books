  ...
  pages: [
    {
      ...
      characterActionPrompt:
        'POSITION — CRITICAL: Place the character in the right half of the image. The image is a 2:1 landscape spread — the left half is the back cover and must remain completely empty.\n\nHORIZONTAL: Her horizontal center must be at exactly 75% from the left edge of the full canvas (≈x=1330 on a 1774px canvas). Do NOT place her near the right canvas edge.\n\nVERTICAL POSITION — CRITICAL: Her head top must start at exactly 31% from the top of the canvas (≈y=275 on an 887px canvas). This places her head clearly BELOW both the title and subtitle text. There must be a visible gap between the bottom of the subtitle and the top of her hair. Do NOT place her head any higher than 31%.\n\nCROP — CRITICAL: Show her from face to waist ONLY. Her full body extends far below the canvas; the bottom edge of the canvas cuts across her at the hip/waist line. Her feet, shins, and knees are completely outside the frame — NOT visible. Her hands holding the basket appear near the very bottom edge of the canvas.\n\nSCALE: She must be large and dominant — filling the majority of the right half. Her head width should occupy approximately 28–32% of the canvas width.\n\nPOSE: Relaxed, 3/4 angle toward the viewer, warm soft smile, both hands holding a wicker flower-girl basket at waist height (visible at the very bottom of the frame).',
      includesSenderCharacter: false,
      // Character is face-to-waist only: head at y=275 (31% from top, below subtitle),
      // canvas bottom (y=887) cuts at the waist. Width ≈30% of canvas centered at x=1330.
      characterPlacement: { x: 1090, y: 275, width: 480, height: 612 },
